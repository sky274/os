...
start_process (void *file_name_)
{
  ...
  // scott
  /* initialize the supplemental page table used to track this process's pages */
  pagesup_init();
  success = load (file_name, &if_.eip, &if_.esp, num_args);
...
process_exit (void)
{
  ...
  /* remove this thread's entries in the frame table */
  if(has_frame_lock())
    release_frame_lock();

  clear_frames();

  /* free the resources associated with the supplementary page table
  and clears out the swap space */
  pagesup_destroy();
...
clear_filelist(){
  ...
  if(!list_empty(filelist))
  {
    ...
    while(e != list_end(filelist)){
      ...
      /* free the child_elem to prevent memory leaks */
      file_close(file_e->file_pointer);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      /* Calculate how to fill this page.
         add a mapping for this page to the
         supplemental page table. This is done so
         executables can be lazy loaded */
      size_t page_read_bytes = read_bytes < PGSIZE ? read_bytes : PGSIZE;
      size_t page_zero_bytes = PGSIZE - page_read_bytes;

      /* Add the page to the supplemental page table */
      /* record the source, the position/offset, and the size */
      pagesup_add(file, ofs, page_read_bytes, upage, writable);
...
setup_stack (void **esp, const char * file_name, int num_args) 
{
  ...
  *esp = PHYS_BASE;
  /* add the first stack page to the supplemental page table
  so it can be allocated when it faults; lazy loading*/
  pagesup_add(NULL, 0, PGSIZE, upage, true);
